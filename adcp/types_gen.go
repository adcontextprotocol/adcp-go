...
// AdCP schema version: v3.0.0-rc.3
...
// AdcpDomain — AdCP protocol domains for task categorization
...
// DaastTrackingEvent — Standard DAAST tracking events for audio ad playback and interaction
...
// VastTrackingEvent — Standard VAST tracking events for video ad playback and interaction
...
const (
	...
	VastTrackingEventClick VastTrackingEvent = "click"
...
type Format struct {
	FormatID any `json:"format_id"` // Structured format identifier with agent URL and format name
	...
	InputFormatIds []any `json:"input_format_ids,omitempty"` // Array of format IDs this format accepts as input creative manifests. When presen
	OutputFormatIds []any `json:"output_format_ids,omitempty"` // Array of format IDs that this format can produce as output. When present, indica
	...
	PricingOptions []any `json:"pricing_options,omitempty"` // Pricing options for this format. Used by transformation and generation agents th
}
...
type CreativeAsset struct {
	...
	FormatID any `json:"format_id"` // Format identifier specifying which format this creative conforms to. Can be: (1)
...
type CreativeManifest struct {
	FormatID any `json:"format_id"` // Format identifier this manifest is for. Can be a template format (id only) or a 
...
type Account struct {
	...
	Brand any `json:"brand,omitempty"` // Brand reference identifying the advertiser
...
type GetAdcpCapabilitiesResponse struct {
	...
	SupportedProtocols []string `json:"supported_protocols"` // Which AdCP domain protocols this seller supports
	Account any `json:"account,omitempty"` // Account management capabilities. Describes how accounts are established, what bi
	MediaBuy any `json:"media_buy,omitempty"` // Media-buy protocol capabilities. Only present if media_buy is in supported_proto
...
type SyncAccountsRequest struct {
	...
	Accounts []any `json:"accounts"` // Advertiser accounts to sync
...
type SyncGovernanceRequest struct {
	AdcpMajorVersion int `json:"adcp_major_version,omitempty"` // The AdCP major version the buyer's payloads conform to. Sellers validate against
	Accounts []any `json:"accounts"` // Per-account governance agent configuration. Each entry pairs an account referenc
	Context any `json:"context,omitempty"`
	Ext any `json:"ext,omitempty"`
}
...
type GetProductsRequest struct {
	...
	Brand any `json:"brand,omitempty"` // Brand reference for product discovery context. Resolved to full brand identity a
	Catalog any `json:"catalog,omitempty"` // Catalog of items the buyer wants to promote. The seller matches catalog items ag
	Account any `json:"account,omitempty"` // Account for product lookup. Returns products with pricing specific to this accou
...
type CreateMediaBuyRequest struct {
	...
	Account any `json:"account"` // Account to bill for this media buy. Pass a natural key (brand, operator, optiona
	...
	Packages []any `json:"packages,omitempty"` // Array of package configurations. Required when not using proposal_id. When execu
	Brand any `json:"brand"` // Brand reference for this media buy. Resolved to full brand identity at execution
...
type GetMediaBuysRequest struct {
	AdcpMajorVersion int `json:"adcp_major_version,omitempty"` // The AdCP major version the buyer's payloads conform to. Sellers validate against
	Account any `json:"account,omitempty"` // Account to retrieve media buys for. When omitted, returns data across all access
...
type GetMediaBuyDeliveryRequest struct {
	AdcpMajorVersion int `json:"adcp_major_version,omitempty"` // The AdCP major version the buyer's payloads conform to. Sellers validate against
	Account any `json:"account,omitempty"` // Filter delivery data to a specific account. When omitted, returns data across al
...
type ListCreativeFormatsRequest struct {
	AdcpMajorVersion int `json:"adcp_major_version,omitempty"` // The AdCP major version the buyer's payloads conform to. Sellers validate against
	FormatIds []any `json:"format_ids,omitempty"` // Return only these specific format IDs (e.g., from get_products response)
	...
	OutputFormatIds []any `json:"output_format_ids,omitempty"` // Filter to formats whose output_format_ids includes any of these format IDs. Retu
	InputFormatIds []any `json:"input_format_ids,omitempty"` // Filter to formats whose input_format_ids includes any of these format IDs. Retur
	Pagination any `json:"pagination,omitempty"`
	Context any `json:"context,omitempty"`
	Ext any `json:"ext,omitempty"`
}
...
type SyncCatalogsRequest struct {
	AdcpMajorVersion int `json:"adcp_major_version,omitempty"` // The AdCP major version the buyer's payloads conform to. Sellers validate against
	Account any `json:"account"` // Account that owns these catalogs.
	Catalogs []any `json:"catalogs,omitempty"` // Array of catalog feeds to sync (create or update). When omitted, the call is dis
...
type BuildCreativeRequest struct {
	...
	TargetFormatID any `json:"target_format_id,omitempty"` // Single format ID to generate. Mutually exclusive with target_format_ids. The for
	TargetFormatIds []any `json:"target_format_ids,omitempty"` // Array of format IDs to generate in a single call. Mutually exclusive with target
	Account any `json:"account,omitempty"` // Account reference for pricing and billing. When present, the creative agent appl
	Brand any `json:"brand,omitempty"` // Brand reference for creative generation. Resolved to full brand identity (colors
...
type SyncCreativesRequest struct {
	AdcpMajorVersion int `json:"adcp_major_version,omitempty"` // The AdCP major version the buyer's payloads conform to. Sellers validate against
	Account any `json:"account"` // Account that owns these creatives.
	Creatives []any `json:"creatives"` // Array of creative assets to sync (create or update)
...
type ListCreativesRequest struct {
	...
	Account any `json:"account,omitempty"` // Account reference for pricing and access. When provided with include_pricing, th
...
type GetSignalsRequest struct {
	AdcpMajorVersion int `json:"adcp_major_version,omitempty"` // The AdCP major version the buyer's payloads conform to. Sellers validate against
	Account any `json:"account,omitempty"` // Account for this request. When provided, the signals agent returns per-account p
...
type ActivateSignalRequest struct {
	...
	Destinations []any `json:"destinations"` // Target destination(s) for activation. If the authenticated caller matches one of
...
// ComplyTestControllerRequest is a discriminated union — use the appropriate variant type.
type ComplyTestControllerRequest = any

// ListScenarios — Discover which scenarios this seller supports
...
// ForceCreativeStatus — Transition a creative to the specified status
type ForceCreativeStatus struct {
	Scenario string `json:"scenario"`
...
// ForceAccountStatus — Transition an account to the specified status
...
// ForceMediaBuyStatus — Transition a media buy to the specified status
type ForceMediaBuyStatus struct {
	Scenario string `json:"scenario"`
	Params any `json:"params"`
	Context any `json:"context,omitempty"`
	Ext any `json:"ext,omitempty"`
...
// ForceSessionStatus — Transition an SI session to a terminal status
type ForceSessionStatus struct {
	Scenario string `json:"scenario"`
	Params any `json:"params"`
	Context any `json:"context,omitempty"`
	Ext any `json:"ext,omitempty"`
...
// SimulateDelivery — Inject synthetic delivery data for a media buy
type SimulateDelivery struct {
	Scenario string `json:"scenario"`
	Params any `json:"params"`
	Context any `json:"context,omitempty"`
	Ext any `json:"ext,omitempty"`
...
// SimulateBudgetSpend — Simulate budget consumption to a specified percentage
type SimulateBudgetSpend struct {
	Scenario string `json:"scenario"`
	Params any `json:"params"`
	Context any `json:"context,omitempty"`
	Ext any `json:"ext,omitempty"`

...
// ForecastableMetric — Standard delivery and engagement metric names for forecasts. **Most values mirro
...
type Product struct {
	...
	OutcomeMeasurement any `json:"outcome_measurement,omitempty"` // **Deprecated as of this minor.** Outcome capabilities (incremental sales lift, b
	DeliveryMeasurement any `json:"delivery_measurement,omitempty"` // Measurement vendors and methodology for delivery metrics. The buyer accepts the 
...
type Package struct {
	...
	CommittedMetrics []map[string]any `json:"committed_metrics,omitempty"` // The binding reporting contract for this package — what the seller has agreed to 
...
type DeliveryTotals struct {
	...
	VendorMetricValues []any `json:"vendor_metric_values,omitempty"` // Reported values for vendor-defined metrics that the product's `reporting_capabil
}
...
type Account struct {
	...
	GovernanceAgents []any `json:"governance_agents,omitempty"` // Governance agent endpoint registered on this account. Exactly one entry per sync
...
type GetAdcpCapabilitiesResponse struct {
	...
	Identity any `json:"identity,omitempty"` // Operator identity posture — trust-root pointer (`brand_json_url`) plus key-scopi
...
type ListAccountsResponse struct {
	Accounts []any `json:"accounts"` // Array of accounts accessible to the authenticated agent. Each entry is the full 
	Errors []AdcpError `json:"errors,omitempty"` // Task-specific errors and warnings
...
type GetProductsResponse struct {
	...
	FilterDiagnostics any `json:"filter_diagnostics,omitempty"` // Optional non-fatal diagnostic block describing how the request's `filters` narro
	Pagination *PaginationResponse `json:"pagination,omitempty"`
...
type ComplyTestControllerRequest struct {
	...
	Context any `json:"context,omitempty"`
	Ext any `json:"ext,omitempty"`
	Account any `json:"account,omitempty"` // Sandbox account assertion. The runner SHOULD set sandbox: true on every comply_t
}
...
type UpstreamTrafficSuccess struct {
	Success bool `json:"success"`
	RecordedCalls []any `json:"recorded_calls"` // Outbound HTTP calls caused by the requesting principal in the requested window, 

...
// TestMiddlewareObserveOnlyBodyOverLimitPreservesFullStream confirms that when
// ObserveOnly is true and the request body exceeds MaxBodyBytes the downstream
// handler still receives the complete unmodified body. Prior to the fix,
// readAndReplaceBody consumed limit+1 bytes from the original body before
// returning the size error, leaving ObserveOnly with a truncated stream to forward.
// Reproduction: body "0123456789abcdef" (16 bytes), limit 8 — handler previously
// received only "9abcdef".
func TestMiddlewareObserveOnlyBodyOverLimitPreservesFullStream(t *testing.T) {
	...
	handler := mw(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		called = true
		var readErr error
		handlerBody, readErr = io.ReadAll(r.Body)
		require.NoError(t, readErr)
		w.WriteHeader(http.StatusOK)
	}))

	req := httptest.NewRequest("POST", "https://seller.example.com/adcp/op",
	...
	assert.Equal(t, http.StatusOK, w.Code)
	assert.True(t, called, "handler must run under ObserveOnly")
	assert.Equal(t, body, string(handlerBody),
		"downstream handler must receive the complete unmodified body when ObserveOnly passes through a body-size failure")
}

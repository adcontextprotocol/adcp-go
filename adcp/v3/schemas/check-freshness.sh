...
run_lint() {
  if ! python3 "$SCRIPT_DIR/lint.py" --allow-missing-schemas; then
    echo "Schema drift detected. See adcp/v3/schemas/lint.py output above for remediation." >&2
    return 1
  fi
  return 0
}
...
if [ "$PINNED" = "latest" ]; then
  ...
  if [ "$PINNED_HASH" = "$UPSTREAM" ]; then
    ...
    echo "Stale. Run: cd adcp/v3/schemas && ./download.sh && python3 generate.py > ../types_gen.go"
    stale=1
  fi
  lint_rc=0
  run_lint || lint_rc=$?
...
if [ -z "$LATEST" ] || [ "$PINNED" = "$LATEST" ]; then
  ...
  echo "Stale. Run: cd adcp/v3/schemas && ./download.sh $LATEST && python3 generate.py > ../types_gen.go"
  stale=1
fi
lint_rc=0
run_lint || lint_rc=$?

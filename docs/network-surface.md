...
**Decoder coverage:** MAID, HashedEmail, and ID5 have format-only decoders that need no external dependency. RampID and RampIDDerived are decoded via the LiveRamp sidecar when `LIVERAMP_SIDECAR_URL` is configured. UID2 and EUID are decoded via the built-in native `uid2client` when the corresponding operator credentials are configured (`UID2_API_KEY` + `UID2_CLIENT_SECRET`, or the `EUID_*` equivalents). UID types without a registered decoder are silently dropped from both the TMPX wire and the audience/fcap shadow request; the startup log enumerates which UID types are dropped.

**UID2 / EUID operator clients:** the agent maintains one client per scope. Each client performs an AES-GCM envelope handshake against the operator (default `https://prod.uidapi.com` for UID2, `https://prod.euid.eu` for EUID), caches the returned keyset in memory, and decrypts advertising tokens locally on the request path — no HTTP call on the hot path. A background goroutine refreshes the keyset every `UID2_KEY_REFRESH_INTERVAL` / `EUID_KEY_REFRESH_INTERVAL` (default 5 min); the refresh HTTP call is bounded by `UID2_HTTP_TIMEOUT` / `EUID_HTTP_TIMEOUT` (default 5s). Partial credentials (API key set but client secret missing, or vice versa) fail startup — operators either set both or leave both unset.

**UID2 / EUID network egress:** the agent makes outbound HTTPS calls to the operator URL on the configured refresh cadence. Production deployments should use the public operator URLs over TLS; the client accepts `http://` for local test operators but rejects any URL without an explicit scheme.

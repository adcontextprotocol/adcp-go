// Copy of targeting/internal/clusterslot for use in the bench seeder.
// The seeder needs the SAME slot→shard mapping identity-agent uses at
// read time so writes land where reads look. Internal packages can't
// be imported from outside their subtree, so this file is a verbatim
// port of that algorithm (CRC-16/XMODEM + valkey-cli --cluster create
// slot allocation).
//
// Keep in lockstep with targeting/internal/clusterslot/clusterslot.go.
package main
...
// shardMap routes keys to shard ordinals using the slot allocation that

...
// the context-agent polls via TMP_REGISTRY_URL. It serves a single-property
// snapshot advertising the loadgen's Ed25519 public key so the agent's
// RemoteKeyStore can resolve X-AdCP-Key-Id → public key on every signed
// request without touching a real registry.
//

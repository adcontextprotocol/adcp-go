...
build_services=(identity-agent configserver seeder loadgen)
if [[ "$SIGN_REQUESTS" == "true" ]]; then
  ...
  for cfg in "${CONFIGS[@]}"; do
    ...
    if [[ "$SIGN_REQUESTS" == "true" ]]; then
      IDENTITY_CPUS="$cpus" IDENTITY_MEMORY="$memory" \
        docker compose --profile baseline --profile signed up -d tmpregistry
    fi
    IDENTITY_CPUS="$cpus" \
    IDENTITY_MEMORY="$memory" \
      docker compose --profile baseline "${SIGNED_PROFILE[@]}" up -d audience-valkey fcap-valkey configserver
    IDENTITY_CPUS="$cpus" \
    IDENTITY_MEMORY="$memory" \
      docker compose --profile baseline "${SIGNED_PROFILE[@]}" up -d --force-recreate identity-agent
    ...
    for qps in "${QPS_STEPS[@]}"; do
      ...
      if [[ -f "$host_report" ]]; then
        ...
        python3 - "$host_report" "$scenario" "$SIGNED_LABEL" "$cpus" "$memory" "$CONCURRENCY" \
          ...
          >> "$SUMMARY_CSV" <<'PY'
import json, sys
p, scenario, signed, cpus, memory, conc, id_rss, aud_rss, fc_rss, id_cpu, aud_cpu, fc_cpu = sys.argv[1:]
r = json.load(open(p))

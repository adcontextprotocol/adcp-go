...
func TestSnapshotVerifiesLoadgenSignature(t *testing.T) {
	...
	req := &tmproto.IdentityMatchRequest{
		...
		Identities: []tmproto.IdentityToken{
			{UIDType: tmproto.UIDType("maid"), UserToken: "0123456789abcdef0123456789abcdef"},
		},
		PackageIDs: []string{"pkg-00001"},
	}
	provider := "http://identity-agent:8080/identity"
	epoch := tmproto.CurrentEpoch()
	sig, err := signer.SignIdentityMatch(req, provider, epoch)
	if err != nil {
		t.Fatalf("SignIdentityMatch: %v", err)
	}

	if err := tmproto.VerifyIdentityMatch(req, provider, sig, kid, ks, time.Now()); err != nil {
		t.Fatalf("VerifyIdentityMatch: %v", err)
	}
}

...
// verified-identity surface. It lives in its own reference module so its
// World-ID-specific dependency surface (an HTTP client to World's verifier
// backend) stays out of the core adcp-go module that every consumer imports —
// the reason targeting.AttestationVerifier is a pluggable interface with no
// default.
//

...
// GLIDE client wired to a fresh Store. The test is skipped when Docker
// isn't reachable so the suite still runs in environments without it.
...
func TestIntegration_Cursor(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Empty(t, cur, "fresh store should have empty cursor")
	...
	// Save empty deletes the cursor — Run() relies on this to clear stale
	// state during cursor-expired re-bootstrap.
...
func TestIntegration_Properties(t *testing.T) {
	...
	// Idempotency: repeating a Put is a no-op semantically.
	require.NoError(t, store.PutProperty(ctx, p1))
...
func TestIntegration_Auth(t *testing.T) {
	...
	// Idempotent Put.
	require.NoError(t, store.PutAuth(ctx, entry1))

	loaded, err := store.LoadAuth(ctx)
	require.NoError(t, err)
	assert.Len(t, loaded, 4)

	// RemoveAuthEntry clears every authorization_type for (agent, domain).
	...
	loaded, err = store.LoadAuth(ctx)
	require.NoError(t, err)
	// entry3 (a → other) and entry4 (b → pub) should remain.
	assert.Len(t, loaded, 2)

	// RemoveAuthAgent removes everything else for agent a.
...
func TestIntegration_KeyPrefixIsolation(t *testing.T) {
	...
	// Pull the underlying client out of the base store to share with a
	// second store under a different prefix.

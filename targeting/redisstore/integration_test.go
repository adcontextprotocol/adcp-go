...
func TestIntegration_FcapStore_BatchWriteAndRead(t *testing.T) {
	...
	lookups := []fcap.CapLookup{
		{UserIdentity: "user-0", Field: fcap.Field{SellerAgentURL: "https://s1", PackageID: "pkg-a"}},
		{UserIdentity: "user-3", Field: fcap.Field{SellerAgentURL: "https://s1", PackageID: "pkg-b"}},
		{UserIdentity: "user-4", Field: fcap.Field{SellerAgentURL: "https://s2", PackageID: "pkg-a"}},
		{UserIdentity: "user-99", Field: fcap.Field{SellerAgentURL: "https://s1", PackageID: "pkg-a"}},
	}
...
func TestIntegration_TargetingStore_SetOps(t *testing.T) {
	...
	_, err := client.SAdd(ctx, "colors", "red", "green", "blue").Result()
	require.NoError(t, err)
	_, err = client.SAdd(ctx, "warm", "red", "orange", "yellow").Result()
	require.NoError(t, err)
	...
	t.Run("SetIntersect_NoOverlap", func(t *testing.T) {
		_, err := client.SAdd(ctx, "cold", "navy", "purple").Result()
		require.NoError(t, err)
...
func TestIntegration_TargetingStore_ExistsAcrossTypes(t *testing.T) {
	...
	_, err := client.SAdd(ctx, "set-key", "member").Result()
	require.NoError(t, err)
